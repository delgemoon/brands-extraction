...
if __name__ == '__main__':
    train_full_models()
    thres_res, max_val = find_threshold('B')
    print(thres_res);print(max_val)
    thres_res, max_val = find_threshold('M')
    print(thres_res);print(max_val)
#     tag a sequence
#     nor_text = sys.argv[1]
#     eng_text = sys.argv[2]
...
#     eng_text = eng_text.encode('utf-8', 'surrogateescape').decode('utf-8')    
    nor_text = 'Hodetelefoner, ikke brukt'
    eng_text = 'white and black sennheiser momentum on -ear ivory headphones'
    prob_B_nor, choice_B_nor, label_B_nor,prob_M_nor, choice_M_nor, label_M_nor = BM_nor.tag_seq('models/crf_nor', [nor_text]) 
    print(label_B_nor); print(prob_B_nor)   
    prob_B_eng, choice_B_eng, label_B_eng,prob_M_eng, choice_M_eng, label_M_eng = BM_eng.tag_seq('models/crf_eng', [eng_text])
    print(label_B_eng); print(prob_B_eng)    
