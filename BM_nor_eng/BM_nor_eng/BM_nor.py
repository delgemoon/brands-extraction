...
'word.numbers', 'word.initCap', 'word.form',
'-1.word.numbers','-1.word.initCap', '-1.word.form',
'-2.word.numbers','-2.word.initCap', '-2.word.form',
'-3.word.numbers','-3.word.initCap', '-3.word.form',
'+1.word.numbers', '+1.word.initCap', '+1.word.form', 
'+2.word.numbers', '+2.word.initCap', '+2.word.form', 
'+3.word.numbers', '+3.word.initCap', '+3.word.form']
model_id = 'crf3_test'

def frequent_terms_list (X, freq_threshold):
    vectorizer = CountVectorizer(ngram_range= (1,1),  token_pattern='(?u)\\b\\w+\\b')
    vectorizer.fit(X)
    feature_name = vectorizer.get_feature_names()
    
    nor_stem = SnowballStemmer('norwegian')
...
def word_form_dict_create(dict_file):
    dict_store = pd.read_csv(dict_file, header= None, sep = ' ').values   
    word_form_dict = { word : form for word, form in zip(dict_store[:, 0], dict_store[:, 1])}
    return word_form_dict 
def dict_look_up (dict_file, inputString):
    input_string = inputString.upper()
    if input_string in dict_file:
        return dict_file[input_string]
    elif not re.match('\w+', input_string):
        return 'punc'
    else:
        return 'NA'
...
def token_2_features(tokens_list, i, frequent_terms, features_list, word_form_dict):
    word = tokens_list[i]        
    ...
    if 'word.form' in features_list:
        features.append( 'word.form' + dict_look_up(word_form_dict, word))             
    ...
    if i > 0:
        ...
        if '-1.word.form' in features_list:
            features.append( '-1.word.form' + dict_look_up(word_form_dict, word1))
    ...
    if i > 1:
        ...
        if '-2.word.form' in features_list :
            features.append( '-2.word.form' + dict_look_up(word_form_dict, word2))
    ...
    if i > 2:
        ...
        if '-3.word.form' in features_list :
            features.append( '-3.word.form' + dict_look_up(word_form_dict, word3))
...
def desc_2_tags(X_string, location_string, brand_string, model_string, frequent_terms, features_list, word_form_dict = None):
    tokens_list = wordpunct_tokenize(X_string)    
    y = ["NA" for i in range(len(tokens_list))]
    # brand
    y = desc_2_tag(y, tokens_list, brand_string, "B")
    # location
    y = desc_2_tag(y, tokens_list, location_string, "L")
    # model
    y = desc_2_tag(y, tokens_list, model_string, "M")
    desc_features = [token_2_features(tokens_list, i,frequent_terms, features_list, word_form_dict) for i in range(len(tokens_list))]
    return desc_features, y

def train_crf(model_id, X,y):
    if 'frequent_terms_nor.p' not in  os.listdir('models'):
        print('----- frequent_terms nor not found')    
        frequent_terms = frequent_terms_list(X, 10)
        pickle.dump( frequent_terms, open( "models/frequent_terms_nor.p", "wb" ) )
    else:
        print('---- frequent_terms nor found in models')
        frequent_terms = pickle.load( open( "models/frequent_terms_nor.p", "rb" ) ) 
    word_form_dict = word_form_dict_create('dictionary.csv')
    params = {
    ...
    for i in range(len(X)):        
        desc_features, blm_features = desc_2_tags(X[i], y[i,0],y[i,1], y[i,2], frequent_terms, init_features_list, word_form_dict)        
...
def tag_seq(crf_model_id, X_test):
    ...
    frequent_terms = pickle.load( open( "BM_nor_eng/models/frequent_terms_nor.p", "rb" ) )
    word_form_dict = word_form_dict_create('BM_nor_eng/models/dictionary.csv')
    X_features = []
